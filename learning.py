...
class CurriculumCallback(CheckpointCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if current_phase != self.curriculum_phase:
            ...
            print(
                f"Curriculum progression: Moving to {phase_names[current_phase]} phase"
            )
...
def make_env(bump_practice=False, bump_challenge=False, use_original_obs=False):
    def _init():
        env = CustomEnvWrapper(
            ...
            use_original_obs=use_original_obs,
        )
        return env
...
parser.add_argument(
    "--bump_practice", action="store_true", help="Enable bump practice mode"
)
parser.add_argument(
    "--bump_challenge", action="store_true", help="Enable bump challenge mode"
)
parser.add_argument(
    "--curriculum", action="store_true", help="Enable curriculum learning progression"
)
parser.add_argument(
    "--transfer_learning", action="store_true", help="Start from pretrained model"
)
parser.add_argument(
    "--enhanced_obs",
    action="store_true",
...
if __name__ == "__main__":
    ...
    if args.curriculum:
        ...
        env = SubprocVecEnv(
            [
                make_env(
                    bump_practice=False,
                    bump_challenge=False,
                    use_original_obs=not use_enhanced_obs,
                )
        ...
        elif args.enhanced_obs:
            folder_name = "enhanced_obs_walker_model"
        elif args.bump_practice:
    ...
    if args.curriculum:
        curriculum_callback = CurriculumCallback(
            save_freq=50000, save_path=save_path, name_prefix="curriculum_walker"
        )
    ...
    learning_rate = (
        3e-4 if (args.bump_practice or args.bump_challenge or args.curriculum) else 1e-4
    )
    ...
    if args.enhanced_obs_transfer_learning and os.path.exists(args.pretrained_path):
        ...
        model.learning_rate = learning_rate * 0.5  # Lower learning rate for fine-tuning
        print(
            "Loaded pretrained model successfully - training with original observation space + enhanced rewards"
        )
    ...
    total_timesteps = (
        15000000
        if (args.bump_practice or args.bump_challenge or args.curriculum)
        else 5000000
    )
